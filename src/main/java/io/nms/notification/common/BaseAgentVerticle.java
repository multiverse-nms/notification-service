...
import io.nms.notification.message.Receipt;
import io.nms.notification.message.Result;
import io.nms.notification.message.Specification;
import io.nms.notification.taskmanager.AbstractAgentTask;
import io.nms.notification.taskmanager.AgentTaskListener;
import io.nms.notification.taskmanager.TaskManager;
	...
	private Message processSpecification(Specification spec) {
		...
		if (spec.isPeriodic()) {
			...
			if (tasks.containsKey(taskId)) {
				...
				taskManager.submitPeriodic(task, task.getInitialDelayMs(), task.getTaskPeriodMs());
			}
			rct.setTimestampNow();
			return rct;
		// if when= "now ... future / event"
		} else { 
			if (tasks.containsKey(taskId)) {
				LOG.info("Corresponding task already running.");
				tasks.get(taskId).incrementSpecNbr();
				...
				tasks.put(taskId, task);
				// schedule task and store reference
				taskManager.submitEvent(task, task.getInitialDelayMs());
			}
			rct.setTimestampNow();
			return rct;
		}
