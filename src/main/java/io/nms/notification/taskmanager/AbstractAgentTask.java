...
public abstract class AbstractAgentTask extends Thread {
	...
	protected abstract short executeSpec(AgentTaskListener t);
	...
	public short check() {
		...
		if (taskPeriodMs != 0 && specification.getPeriod() < taskPeriodMs) {
	...
	public void run() {
		...
		// event driven tasks execute this once
		short resultCode = executeSpec(this.tListener);

		// only periodic tasks will get here
		if (resultCode == Errors.TASK_SUCCESS) {
			Timestamp ts = new Timestamp(new Date().getTime());
			this.tListener.onResult(taskId, resultCode, ts);
		} else if (resultCode == Errors.TASK_ERROR) {
